...
public class BookEntryServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        if (!isNotEmpty(isbn) || !isNotEmpty(title) || !isNotEmpty(author) || 
            ...
            response.sendRedirect("/gallardoweb2/error/book/empty_field_error.jsp");
            return;
        }
        if (!isValidISBN(isbn)) {
            LOGGER.warning("Validation failed: Invalid ISBN - %s".formatted(isbn));
            response.sendRedirect("/gallardoweb2/error/book/invalid_isbn.jsp");
            return;
        }
        if (!isValidPrice(priceStr)) {
            LOGGER.warning("Validation failed: Invalid price format - %s".formatted(priceStr));
            response.sendRedirect("/gallardoweb2/error/book/invalid_price.jsp");
            return;
        }
        if (!isValidQuantity(qtyStr)) {
            LOGGER.warning("Validation failed: Invalid quantity format - %s".formatted(qtyStr));
            response.sendRedirect("/gallardoweb2/error/book/invalid_quantity.jsp");
            return;
        }

        // Parsing the price and quantity since validation passed
        ...
        // Set attributes and forward to the handler
